...
function feng__autoload($load_class_name) {
	...
	if(!$loader) {
		...
		// ext/mysql fue eliminado en PHP 7 - se reemplaza por PDO manteniendo el mismo comportamiento
		try {
			$temp_link = new PDO("mysql:host=" . DB_HOST . ";dbname=" . DB_NAME, DB_USER, DB_PASS);
			$stmt = $temp_link->query("SELECT name FROM " . TABLE_PREFIX . "plugins WHERE is_installed = 1 AND is_activated = 1;");
			...
			// sin conexion a la base todavia no podemos indexar plugins, seguimos solo con los directorios base
		}
