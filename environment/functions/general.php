...
function php_config_value_to_bytes($val) {
	...
	$val = (float) $val; // usuwa sufiks jednostki (K/M/G) przed mnożeniem - unika "non-numeric value" w PHP 8
	switch($last) {
...
function fix_input_quotes() {
	// magic_quotes_gpc zostały usunięte w PHP 5.4, funkcja get_magic_quotes_gpc() w PHP 8.0 -
	// magic quotes nie mogą być włączone na żadnym wspieranym PHP, więc to zawsze był no-op
} // fix_input_quotes
